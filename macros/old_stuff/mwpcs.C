...
vector<vector<double> > mwpcs(TClonesArray* twim_tclone, TClonesArray* mw2_tclone, TClonesArray* mw3_tclone,TClonesArray* tof_tclone, TClonesArray* start_tclone){
	...
	if (nr_twim == 2 && nr_mw2 == 2 && nr_mw3 == 2 && nr_tof == 2  && nr_start > 1){
		...
		for (Int_t i = 0; i < 2; i++){
			...
			//instead of using info of mw3, use tofw...
			//v_mw3_y.push_back(softofwhitdata[i]->GetY());
			v_mw2_x.push_back(sofmwpc2hitdata[i]->GetX());
			v_mw3_x.push_back(sofmwpc3hitdata[i]->GetX());
			//instead of using info of mw3, use tofw...
			//v_mw3_x.push_back(softofwhitdata[i]->GetX());
		...
		vector<double> v_mw2_y_not_sorted = v_mw2_y;
		sort(v_mw2_y.begin(),v_mw2_y.end());
		sort(v_mw3_y.begin(),v_mw3_y.end());
		if (!isnan(v_mw2_y[0]) && !isnan(v_mw2_y[1]) && !isnan(v_mw3_y[0]) && !isnan(v_mw3_y[1]) && v_mw2_y[0] != -1000 && v_mw2_y[1] != -1000 && v_mw3_y[0] != -1000 && v_mw3_y[1] != -1000 && !isnan(v_mw2_x[0]) && !isnan(v_mw2_x[1]) && !isnan(v_mw3_x[0]) && !isnan(v_mw3_x[1]) && v_mw2_x[0] != -1000 &&   v_mw2_x[1] != -1000 && v_mw3_x[0] != -1000 && v_mw3_x[1] != -1000){
		...
		if ((fSection1 == 0 || fSection1 == 3) && (fSection2 == 1 || fSection2 == 2)){ //charge1 down, charge2 up
			if (v_mw2_y_not_sorted[0] == v_mw2_y[0]){
			...
			else if (v_mw2_y_not_sorted[1] == v_mw2_y[0]){
				if (v_mw3_y_not_sorted[0] == v_mw3_y[0]){
					mwpc_vector[0][0] = charge_1;
