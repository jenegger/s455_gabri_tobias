...
void unpack_offline(const string& input_str)
{
    const Int_t fRunId = 273;
    const Int_t nev = -1;
    ...
    if (fExpId == 455)
    {
        ...
        // Input file
//	sprintf(filename,"/u/land/tobias_jenegger/s455_03_273_%s_stitched.lmd",count_i);
//
        
	
        filename = string("/u/land/tobias_jenegger/s455_03_273_") + input_str + string("_stitched.lmd");
	cout << "INPUT FILE NAME:\t" << filename << endl; 

        TString outputpath = "/u/land/tobias_jenegger/unpacked_s455_data/";
        // outputFilename = outputpath + "s"+cExpId+"_map_data_offline_" + oss.str() + ".root";
	//sprintf(outputFilename,"s455_03_273_%s_unpacked.root");
        outputFilename = outputpath + string("s455_03_273_") + input_str + string("_unpacked") + string(".root");
